...
def main():
    ...
    dataset["Code"] = dataset["Body"].apply(
        lambda cell: BeautifulSoup(cell, "html.parser").find_all("code")
    )
    dataset["Text"] = dataset["Body"].apply(
        lambda cell: [
            item.get_text() for item in BeautifulSoup(cell, "html.parser").find_all("p")
        ]
    )
